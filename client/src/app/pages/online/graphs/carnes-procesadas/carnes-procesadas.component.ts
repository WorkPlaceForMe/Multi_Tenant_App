...
@Component({
  selector: 'ngx-carnes-procesadas',
  templateUrl: './carnes-procesadas.component.html',
  styleUrls: ['./carnes-procesadas.component.scss']
})
export class CarnesProcesadasComponent implements OnInit , OnDestroy {
  @Input() range: NbCalendarRange<Date>;
  @Input() camera;
  carnesProcesadas: any = [];
  player: any;
  timezone: any;
  now_user: Account;
      ...
      (res) => {
        ...
        for (var m of this.carnesProcesadas.raw) {
          ...
          m["clip_path"] =
            ...
            "/" +
            m["clip_path"];
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/carnesProcesadas/' + m['cam_id'] + '/' + m['movie']);
        ...
        let labels = [];
        for (var o of Object.keys(this.carnesProcesadas.over)) {
          o = o + ":00:00";
          labels.push(this.datepipe.transform(o, "yyyy-M-dd HH:mm"));
        }
        this.source = this.carnesProcesadas.raw.slice().sort((a, b) => +new Date(b.time) + +new Date(a.time));
            ...
            datasets: [
              {
                ...
                data: Object.values(this.carnesProcesadas.over),
