...
export class DemographicsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.demographics(this.camera, l).subscribe(
      (res) => {
        this.demographics = res["data"];
        for (var m of this.demographics.raw) {
          ...
          m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/Demograph_fr/' + m['cam_id'] + '/' + m['movie']);
          m["clip_path"] =
            api +
            "/pictures/" +
            this.now_user["id_account"] +
        ...
        this.source = this.demographics.raw
        //   .slice()
        //   .sort((a, b) => +new Date(b.time) - +new Date(a.time));
        let labels = [];
        for (var o of Object.keys(this.demographics.over)) {
          o = o + ":00:00";
            ...
            datasets: [
              {
                ...
                data: Object.values(this.demographics.over),
