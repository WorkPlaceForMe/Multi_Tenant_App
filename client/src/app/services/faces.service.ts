...
return this.http.post(`${this.API_URI}/images`, image);
  ...
  updateRelation(id: string, updateRelation: Relation): Observable<Relation>{
    return this.http.put(`${this.API_URI}/relations/edit/${id}`, updateRelation);
  }
  gethm1(start: string, end:string, camera_id:string){
    return this.http.get(`${this.API_URI}/heatmap/date/${start}/${end}/${camera_id}`);
  }
 ...
 deleteSomeImages(images:any, user_id:string){
  ...
  delVid(data){
    return this.http.delete(`${this.API_URI}/elastic/video/delete/`, data);
  }
}
