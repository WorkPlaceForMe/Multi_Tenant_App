...
exports.tamper = async (req, res) => {
  ...
  await db.con().query(`SELECT * from alerts WHERE alert= 'tamper' and ${data.type} = '${req.params.id}' and time >= '${data.start}' and  time <= '${data.end}' order by time asc;`, function (err, result) {
    if (err) return res.status(500).json({
      success: false,
      message: err
    });
...
exports.direction = async (req, res) => {
  ...
  const data = req.body;
  await db.con().query(`SELECT * from direction WHERE ${data.type} = '${req.params.id}' and time >= '${data.start}' and  time <= '${data.end}' order by time asc;`, function (err, result) {
    if (err) return res.status(500).json({
      success: false,
      message: err
    });
...
exports.speeding = async (req, res) => {
  ...
  const data = req.body;
  await db.con().query(`SELECT * from speed WHERE ${data.type} = '${req.params.id}' and time >= '${data.start}' and  time <= '${data.end}' order by time asc;`, function (err, result) {
    if (err) return res.status(500).json({
      success: false,
      message: err
    });
