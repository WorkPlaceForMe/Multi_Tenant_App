...
const path = process.env.home + process.env.username + process.env.pathDocker + process.env.resources
  ...
  exports.addFr = async (req, res) =>{
    ...
    fs.writeFile(pathPic + '/attr.json', JSON.stringify(user));
    // const file = await fs.promises.readFile(path + '/info.json', 'utf8');
    res.status(200).send({ success: true ,message: "User was registered successfully!", id: user.uuid });
  });
  }
  ...
  exports.deleteUser = (req, res) => {
    ...
    fs.rmdir(folder, {recursive: true}, (err) => {
      if(err) {res.status(500).send({ message: err.message })}
          ...
          res.status(200).json({ success: true, message: 'Successfully deleted folder', id: req.params.uuid })
      }
  })
})
  }