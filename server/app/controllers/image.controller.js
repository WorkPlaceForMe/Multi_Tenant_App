    ...
    try{
        ...
        try {
            new ExifImage({ image : `${pathPic}/${name}`}, function (error, exifData) {
                if (error){
                    res.status(200).send({ success: true ,message: "Image error: " + error.message + ". But was uploaded", name: name });
                }
                else
                    if(exifData.image.Orientation == 6){
                        const form = name.split('.')
                        cp.exec('ffmpeg -y -i ' + pathPic + '/'+ form[0] + '.' + form[1] + ' -vf transpose=1 ' + pathPic + '/'+ form[0] + '_1.' + form[1], function(err,data){
                            console.log('err: ',err)
    ...
    const files = fs.readdirSync(`${path}${decoded.id_account}/${decoded.id_branch}/pictures/${id}`);
        for(let i=0; i <files.length; i ++){
            ...
            const file = fs.statSync(fileName);
                    if(file.isFile()){
                        if(files[i].includes('.jpg') || files[i].includes('.png') || files[i].includes('.JPG') || files[i].includes('.jpeg') || files[i].includes('.PNG')){
